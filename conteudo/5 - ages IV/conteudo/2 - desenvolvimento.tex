...
\subsection{Arquitetura Utilizada}
  ...
  Em termos de fluxo, o usuário acessa o \textit{frontend} no navegador, que envia requisições para a \ac{api}. O \textit{backend} processa essas requisições e, quando necessário, consulta o MongoDB, manipula arquivos no \ac{s3} e busca segredos no Secrets Manager. Depois disso, a resposta retorna ao \textit{frontend} para exibição ao usuário final. A solução final ainda não está implementada, então seguimos com a arquitetura planejada para garantir que o desenvolvimento fique alinhado com as necessidades do projeto e dos \textit{stakeholders}.
